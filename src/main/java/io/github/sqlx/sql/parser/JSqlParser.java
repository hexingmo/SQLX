...
import io.github.sqlx.sql.SqlAttributeBuilder;
...
public class JSqlParser implements SqlParser {
    ...
    private static class SqlAttributeVisitor extends TablesNamesFinder implements SqlAttributeBuilder<Statement> {
        ...
        private final Set<io.github.sqlx.sql.Table> tableSet = new HashSet<>();

        private final Set<String> subTables = new HashSet<>();

        private final Set<io.github.sqlx.sql.Table> subTableSet = new HashSet<>();

        private final Set<String> joinTables = new HashSet<>();

        private final Set<io.github.sqlx.sql.Table> joinTableSet = new HashSet<>();

        private final Set<String> fromTables = new HashSet<>();

        private final Set<io.github.sqlx.sql.Table> fromTableSet = new HashSet<>();

        private final Set<String> insertTables = new HashSet<>();

        private final Set<io.github.sqlx.sql.Table> insertTableSet = new HashSet<>();

        private final Set<String> updateTables = new HashSet<>();

        private final Set<io.github.sqlx.sql.Table> updateTableSet = new HashSet<>();

        private final Set<String> deleteTables = new HashSet<>();

        private final Set<io.github.sqlx.sql.Table> deleteTableSet = new HashSet<>();

        private final Set<io.github.sqlx.sql.Table> readTableSet = new HashSet<>();

        private final Set<io.github.sqlx.sql.Table> writeTableSet = new HashSet<>();
        ...
        public DefaultSqlAttribute build(Statement obj) {
            ...
            prevVisited = null;
            for (io.github.sqlx.sql.Table table : tableSet) {
        ...
        public void visit(Insert insert) {
            ...
            io.github.sqlx.sql.Table tbe = new io.github.sqlx.sql.Table(table.getFullyQualifiedName(), database, tableName);
        ...
        public void visit(Table table) {
            ...
            io.github.sqlx.sql.Table tbe = new io.github.sqlx.sql.Table(table.getFullyQualifiedName(), database, table.getName());
            tableSet.add(tbe);
        ...
        public void visit(SubSelect subSelect) {
            ...
            if (selectBody instanceof PlainSelect) {
                ...
                if (select.getFromItem() instanceof Table) {
                    ...
                    io.github.sqlx.sql.Table tbe = new io.github.sqlx.sql.Table(table.getFullyQualifiedName(), tableName, table.getName());
                    subTableSet.add(tbe);
                }
            }
        }
